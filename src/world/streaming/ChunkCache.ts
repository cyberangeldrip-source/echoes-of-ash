...
export class ChunkCache {
  ...
  public coordinates(): readonly ChunkCoordinate[] {
    return [...this.#entries.values()].map(entry => entry.chunk.coordinate);
  }
  ...
  public peek(coordinate: ChunkCoordinate): GeneratedChunk | undefined {
    return this.#entries.get(chunkKey(coordinate))?.chunk;
  }
  ...
  public enforceBudget(): readonly GeneratedChunk[] {
    return this.#evict();
  }
  ...
  #evict(): readonly GeneratedChunk[] {
    ...
    while (this.#bytes > this.#maximumBytes) {
      const candidate = [...this.#entries.entries()].filter(([, entry]) => !entry.pinned).sort((left, right) => left[1].lastAccess - right[1].lastAccess || left[0].localeCompare(right[0]))[0];
  ...
  #measure(chunk: GeneratedChunk): number {
    return chunk.voxels.byteLength + chunk.surfaceBiomes.byteLength;
  }
}
